...
class DoctrineCheck implements CheckInterface
{
    ...
    public function check(string $component): Response
    {
        if ($this->container->has('doctrine.orm.entity_manager') === false) {
            return new Response(self::CHECK_RESULT_NAME, 'fail', 'Entity Manager Not Found.', '0.2');
        }

        $entityManager = $this->container->get('doctrine.orm.entity_manager');

        if ($entityManager === null) {
            return new Response(self::CHECK_RESULT_NAME, 'fail', 'Entity Manager Not Found.', '0.3');
        }
